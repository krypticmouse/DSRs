use crate::{
    Evaluator, Example, LM, Module, Optimizable, Optimizer, Predict, Prediction,
    Predictor, example, get_lm,
};
use anyhow::Result;
use bon::Builder;
...
struct GenerateInstructionGivenAttempts {
    ...
    #[input(desc="The instructions I've tried, along with their corresponding validation scores")]
...
impl COPRO {
    fn get_output_field_prefix(&self, predictor: &dyn Optimizable) -> String {
        ...
        if let Some(obj) = output_fields.as_object() {
            if let Some((_, field)) = obj.iter().last() {
...
impl Optimizer for COPRO {
    async fn compile<M: Module + Optimizable + Evaluator>(
        ...
        let predictor_info: Vec<(String, String, String)> = {
            ...
            named_predictors.iter().map(|(name, predictor)| {
        ...
        for (predictor_name, basic_instruction, basic_prefix) in &predictor_info {
            ...
            if self.breadth > 1 {
                let mut futures: Vec<Pin<Box<dyn Future<Output = Result<Prediction>> + Send>>> = Vec::new();
                ...
                for _ in 0..self.breadth - 1 {
                    ...
                    if let Some(mut prompt_model) = self.prompt_model.clone() {
                        ...
                        futures.push(Box::pin(async move {
                            BASIC_GENERATOR.forward_with_config(
                                example! {
                ...
                for pred in predictions {
                    let instruction = pred.data.get("proposed_instruction")
                        .and_then(|v| v.as_str())
                        .unwrap_or(&basic_instruction)
                        .to_string();
                    let prefix = pred.data.get("proposed_prefix_for_output_field")
                        .and_then(|v| v.as_str())
        ...
        for d in 0..self.depth {
            ...
            for (p_i, (predictor_name, _, _)) in predictor_info.iter().enumerate() {
                ...
                for (c_i, (instruction, prefix)) in candidates_to_eval.iter().enumerate() {
                    ...
                    let score = if let Some(existing) = evaluated_candidates
                        ...
                        println!(
                            ...
                            d + 1, self.depth, c_i + 1, candidates_to_eval.len(), 
                            p_i + 1, predictor_info.len()
                        );
                ...
                if let Some(best) = evaluated_candidates
                    .get(predictor_name)
                    .and_then(|m| m.values().max_by(|a, b| a.score.partial_cmp(&b.score).unwrap())) {
                    {
                        let mut module_predictors = module.parameters();
                        if let Some(predictor) = module_predictors.get_mut(predictor_name) {
                ...
                if self.track_stats && !latest_scores.is_empty() {
                    ...
                    stats.results_latest.get_mut(predictor_name).unwrap().push(avg);
                    ...
                    if !best_scores.is_empty() {
                        ...
                        stats.results_best.get_mut(predictor_name).unwrap().push(best_avg);
                    }
                }
            }
            ...
            for (predictor_name, _, _) in &predictor_info {
                ...
                if let Some(mut prompt_model) = self.prompt_model.clone() {
                    ...
                    futures.push(Box::pin(async move {
                        REFINEMENT_GENERATOR.forward_with_config(
                            example! {
                ...
                if let Ok(predictions) = results.into_iter().collect::<Result<Vec<_>>>() {
                    ...
                    for pred in predictions {
                        ...
                        let instructions = if let Some(arr) = pred.data.get("proposed_instruction").and_then(|v| v.as_array()) {
                            arr.iter().filter_map(|v| v.as_str()).map(|s| s.to_string()).collect()
                        } else if let Some(s) = pred.data.get("proposed_instruction").and_then(|v| v.as_str()) {
                            vec![s.to_string()]
                        } else {
                            vec![]
                        };
                        
                        let prefixes = if let Some(arr) = pred.data.get("proposed_prefix_for_output_field").and_then(|v| v.as_array()) {
                            arr.iter().filter_map(|v| v.as_str()).map(|s| s.to_string()).collect()
                        } else if let Some(s) = pred.data.get("proposed_prefix_for_output_field").and_then(|v| v.as_str()) {
                            vec![s.to_string()]
                        } else {
                            vec![]
                        };
                    ...
                    all_candidates.get_mut(predictor_name).unwrap().extend(new_candidates.clone());
        ...
        for (predictor_name, candidates_map) in &evaluated_candidates {
            if let Some(best) = candidates_map.values().max_by(|a, b| a.score.partial_cmp(&b.score).unwrap()) {
