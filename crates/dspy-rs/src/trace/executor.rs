...
use crate::{Example, Prediction, GLOBAL_SETTINGS};
            ...
            match &node.node_type {
                NodeType::Root => {
                    ...
                    if node.id == 0 {
                        ...
                        let pred = Prediction::from(
                            root_input.data.iter().map(|(k,v)| (k.clone(), v.clone())).collect::<Vec<_>>()
                        );
                        ...
                        if let Some(data) = &node.input_data {
                             let pred = Prediction::from(
                                data.data.iter().map(|(k,v)| (k.clone(), v.clone())).collect::<Vec<_>>()
                            );
                            node_outputs.insert(node.id, pred);
                        }
                    }
                ...
                NodeType::Predict { signature, .. } => {
                    ...
                    if let Some(parent_id) = node.inputs.first() {
                         if let Some(input_pred) = node_outputs.get(parent_id) {
                ...
                NodeType::Map { mapping } => {
                    ...
                    let result = Prediction::from(data.iter().map(|(k,v)| (k.clone(), v.clone())).collect::<Vec<_>>());
