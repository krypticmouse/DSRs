...
use dsrs_predict::{
    Chat, ChatAdapter, LM, LMClient, Message, Predict, PredictError, Role, Signature,
    TestCompletionModel, ToolLoopMode, configure,
};
...
async fn lm_caller_managed_returns_tool_calls_in_chat_history() {
    ...
    let chat = Chat::new(vec![Message::user("Run some code")]);
    let response = lm
