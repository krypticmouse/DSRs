use dsrs_predict::{ChainOfThought, Facet, ModuleExt, PredictError, ReAct, Signature};
...
fn drop_reasoning(output: dsrs_predict::WithReasoning<QAOutput>) -> QAOutput {
    output.inner
}
...
fn drop_reasoning_checked(
    output: dsrs_predict::WithReasoning<QAOutput>,
...
fn map_shape_exposes_inner_chain_of_thought_shape() {
    let mapped = ChainOfThought::<QA>::new()
        .map(drop_reasoning as fn(dsrs_predict::WithReasoning<QAOutput>) -> QAOutput);
...
fn and_then_shape_exposes_inner_chain_of_thought_shape() {
    let chained = ChainOfThought::<QA>::new().and_then(
        drop_reasoning_checked
            as fn(dsrs_predict::WithReasoning<QAOutput>) -> Result<QAOutput, PredictError>,
    );
