...
| **F4** | **Module trait** — `trait Module { type Input; type Output; async fn forward(&self, input) -> CallOutcome<Output> }`. `CallOutcome` is the single return surface (result + metadata), with trait-based ergonomics for `?`-style consumption so there is no parallel convenience API. All prompting strategies implement this: `Predict<S>`, `ChainOfThought<S>`, `ReAct<S>`, `BestOfN<M>`, `Refine<M>`, user-defined modules. This is the swapping/composition interface. | |
