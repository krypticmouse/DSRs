...
Your field docstring from Chapter 1 became line 1. Your enum became that `one of` block, four legal values, spelled out; this is why `kinda mad` was never going to parse. Those `[[ ## field ## ]]` fences are the **marker protocol**, a convention DSRs inherits from DSPy: the model may think in prose all it wants, but each output field lives between its markers, which is what makes the response mechanically recoverable without forbidding the model from being a language model.
