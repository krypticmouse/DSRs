...
```rust
let otlp = trace.to_otlp_json(true); // include_content: prompts and outputs ride along
```
...
```rust
let rollout = trace.to_rl_rollout(); // prompt/completion transitions plus the score
```
