...
`LM` is built using the builder pattern and holds:
  - `model` - Model identifier (e.g., "gpt-4o-mini", "openai:gpt-4o-mini", or "openai-responses:gpt-4o-mini")
...
|--------------|-----------------|----------------------|---------------------------------------------------------------------------------|
| `model`      | `String`        | `"openai:gpt-4o-mini"` | Supports "provider:model" format or bare model name (defaults to OpenAI chat completions). Use `openai-responses:<model>` for the Responses API. |
...
- `openai-responses` - OpenAI Responses API (requires `OPENAI_API_KEY`)
...
let lm = LM::builder()
    .model("openai-responses:gpt-4o".to_string())
    .build()
    .await?;
