...
```rust
use dsrs_core::{Augmented, Example, Reasoning, WithReasoning};
...
```rust
use dsrs_core::{Module, PredictError, Predicted};
use dsrs_macros::Signature;
use dsrs_predict::{ChainOfThought, Predict};
...
let results = dsrs_core::forward_all(&cot, inputs, 10).await;
