...
GEPA uses rich textual feedback, not just scores, to guide evolutionary search over a Pareto frontier of candidates. Leaven integration scaffolding lives in `dsrs-leaven` as the next optimizer runtime. COPRO and MIPROv2 were removed with the crate split.
