---
title: "Quickstart"
description: "Build your first LM pipeline in minutes"
icon: "rocket"
---

This guide walks you through building a typed LM pipeline with DSRs.

<Steps>
<Step title="Install DSRs">

Add to your `Cargo.toml`:

```toml
[dependencies]
dspy-rs = "0.7"
tokio = { version = "1", features = ["full"] }
anyhow = "1"
```

Or via cargo:

```bash
cargo add dspy-rs tokio anyhow
```

</Step>

<Step title="Configure the LM">

Set up your language model and adapter:

```rust
use dspy_rs::{configure, ChatAdapter, LM};

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    configure(
        LM::builder()
            .model("openai:gpt-4o-mini".to_string())
            .build()
            .await?,
        ChatAdapter,
    );

    Ok(())
}
```

<Note>
The API key is read from `OPENAI_API_KEY` environment variable.
For other providers, use the appropriate prefix (e.g., `anthropic:claude-3-haiku`).
</Note>

</Step>

<Step title="Define a signature">

A [signature](/docs/building-blocks/signature) declares your task's inputs and outputs:

```rust
use dspy_rs::Signature;

/// Answer questions accurately and concisely.
#[derive(Signature, Clone, Debug)]
struct QA {
    /// The question to answer
    #[input]
    question: String,

    /// A clear, direct answer
    #[output]
    answer: String,
}
```

The doc comments become:
- **Struct docstring** → instruction for the LM
- **Field docstrings** → field descriptions in the prompt

</Step>

<Step title="Call the LM">

Create a [predictor](/docs/building-blocks/predictors) and call it:

```rust
use dspy_rs::Predict;

let predict = Predict::<QA>::new();

let output = predict.call(QAInput {
    question: "What is the capital of France?".into(),
}).await?;

println!("Answer: {}", output.answer);
```

That's it - `output.answer` is a typed `String`, not JSON parsing.

</Step>
</Steps>

## Complete example

```rust
use dspy_rs::{configure, ChatAdapter, LM, Predict, Signature};

/// Answer questions accurately and concisely.
#[derive(Signature, Clone, Debug)]
struct QA {
    #[input]
    question: String,

    #[output]
    answer: String,
}

#[tokio::main]
async fn main() -> anyhow::Result<()> {
    configure(
        LM::builder()
            .model("openai:gpt-4o-mini".to_string())
            .build()
            .await?,
        ChatAdapter,
    );

    let predict = Predict::<QA>::new();

    let output = predict.call(QAInput {
        question: "What is the capital of France?".into(),
    }).await?;

    println!("Answer: {}", output.answer);

    Ok(())
}
```

## Next steps

<CardGroup cols={2}>
<Card title="Signatures" icon="pen-to-square" href="/docs/building-blocks/signature">
Define task contracts with typed inputs/outputs
</Card>
<Card title="Custom Types" icon="shapes" href="/docs/building-blocks/types">
Use your own structs and enums in signatures
</Card>
<Card title="Predictors" icon="robot" href="/docs/building-blocks/predictors">
Call LMs with typed signatures, add demos
</Card>
<Card title="Modules" icon="circle-nodes" href="/docs/building-blocks/module">
Compose multi-step pipelines
</Card>
</CardGroup>

## Adding complexity

### Custom types

When you need more than primitives, derive [`BamlType`](/docs/building-blocks/types):

```rust
use dspy_rs::{Signature, BamlType};

#[derive(BamlType, Clone, Debug)]
enum Sentiment {
    Positive,
    Negative,
    Neutral,
}

#[derive(Signature, Clone, Debug)]
/// Analyze the sentiment of the text.
struct SentimentAnalysis {
    #[input]
    text: String,

    #[output]
    sentiment: Sentiment,

    #[output]
    confidence: f64,
}
```

### Few-shot demos

Add examples to guide the LM:

```rust
let predict = Predict::<QA>::builder()
    .demo(QA {
        question: "What is 2+2?".into(),
        answer: "4".into(),
    })
    .build();
```

### Constraints

Validate outputs with [`#[check]` and `#[assert]`](/docs/building-blocks/constraints):

```rust
#[derive(Signature, Clone, Debug)]
struct Rating {
    #[input]
    text: String,

    #[output]
    #[assert("this >= 1 && this <= 5")]
    score: i32,
}
```

### Multi-step pipelines

Compose [modules](/docs/building-blocks/module) for complex workflows:

```rust
struct SummarizeAndRate {
    summarizer: Predict<Summarize>,
    rater: Predict<Rate>,
}

impl Module for SummarizeAndRate {
    async fn forward(&self, inputs: Example) -> anyhow::Result<Prediction> {
        let summary = self.summarizer.forward(inputs).await?;
        // ... chain to rater
    }
}
```
