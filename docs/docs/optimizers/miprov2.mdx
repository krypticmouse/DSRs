...
You can also set `eval_concurrency` (concurrent LM calls during evaluation, default 16) and `seed` (fixes minibatch sampling for reproducible runs). The full field table is in the [optimizers reference](/docs/reference/optimizers#miprov2).
...
The metric is the same `TypedMetric` used by every optimizer: `evaluate(&self, example, prediction, trace) -> Result<Eval>`. MIPROv2 only reads the numerical score; feedback is ignored. See the [evaluation reference](/docs/reference/evaluate) for the trait.

### Typed Data Loading

Use the shared data ingress reference: [`DataLoader`](/docs/reference/data).
