...
impl TypedMetric<MyModule> for ExactMatchMetric {
    async fn evaluate(&self, example: &Example, prediction: &Predicted<QAOutput>) -> Result<MetricOutcome> {
        let expected = example.get("answer", None).as_str().unwrap_or("").trim().to_lowercase();
