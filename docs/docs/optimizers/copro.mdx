...
use bon::Builder;
use dspy_rs::{
    COPRO, Eval, LM, Module, Optimizer, Predict, PredictError,
...
The trainset here is a `Vec` of `(QAInput, QAOutput)` tuples, the zero-boilerplate row form. Any row type that implements `ToInput<QAInput>` works, including `#[derive(Example)]` structs carrying gold labels and metric-only fields; see [Data](/docs/components/data).
