...
impl TypedMetric<MyModule> for MyMetric {
    async fn evaluate(&self, example: &Example, prediction: &Predicted<MySignatureOutput>)
...
let result = gepa.compile(&mut module, my_tasks, &metric).await?;
