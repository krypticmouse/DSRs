...
Unlike score-only prompt search, GEPA uses rich textual feedback to guide candidate evolution:
...
```rust
use dsrs_core::{Example, Module, PredictError};
use dsrs_evaluate::{feedback_helpers, MetricOutcome, TypedMetric};
use dsrs_gepa::GEPAOptimizer;
use dsrs_lm::{ChatAdapter, LM};
...
### Document Retrieval
```rust
use dsrs_evaluate::feedback_helpers::retrieval_feedback;
...
### Code Generation
```rust
use dsrs_evaluate::feedback_helpers::{code_pipeline_feedback, CodeStage, StageResult};
...
```rust
use dsrs_evaluate::feedback_helpers::multi_objective_feedback;
...
A well-designed metric is central to GEPA's sample efficiency. The DSRs implementation expects the metric to return a `MetricOutcome`; for GEPA that means `MetricOutcome::with_feedback(score, FeedbackMetric { ... })`.
...
<Card title="Sentiment Analysis" icon="face-smile" href="https://github.com/krypticmouse/DSRs/blob/main/crates/dsrs-gepa/examples/09-gepa-sentiment.rs">
...
- [Example Code](https://github.com/krypticmouse/DSRs/blob/main/crates/dsrs-gepa/examples/09-gepa-sentiment.rs)
