...
`Example<S>` is the typed input/expected-output pair for a signature: the unit of trainsets, few-shot demos, and metric ground truth. `DataLoader` is the ingestion path that produces them from JSON, CSV, Parquet, and HuggingFace sources; every loader returns `Vec<Example<S>>` directly, so loaded rows flow straight into [`evaluate_trainset`](/docs/components/evaluation) and [`Optimizer::compile`](/docs/components/optimizers) with no manual conversion step. There is no untyped row type in the public contract: custom mappers work with `RowRecord` at the load boundary.
...
- [Predict](/docs/components/predict) for demos on the leaf module
