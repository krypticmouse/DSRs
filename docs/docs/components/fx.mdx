...
| `clear_instruction(name)` | Explicitly resets the name's instruction to the signature default — wins over any instance override when injected ambiently. |
| `set_demos(name, rows)` | Sets the demo rows (flat JSON objects) as an explicit set: an empty vec means "no demos", overriding instance demos. |
...
| `bind(program)` (`ir` feature) | Binds name-keyed params against a compiled `Program` into an `ir::Overlay` — how a module-lane `Candidate` becomes evaluable on a program target. |
| `from_overlay(program, overlay)` (`ir` feature) | The inverse: unbinds an `ir::Overlay` into `Params`. |
