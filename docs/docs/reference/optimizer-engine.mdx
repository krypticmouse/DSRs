...
description: "Reference for EvalEngine, ProgramEvalEngine, Candidate, Budget, RolloutCache, ScoreMatrix, and the Pareto and outcome types every optimizer shares"
icon: "gauge-high"
---

Every optimizer in DSRs (COPRO, GEPA, MIPROv2, bootstrap) is a thin strategy over one shared evaluation core. That core exists in two lanes:
...
- [Optimizers reference](/docs/reference/optimizers)
- [Evaluate reference](/docs/reference/evaluate)
- [Runtime reference](/docs/reference/runtime) for the IR `Budget`, `BudgetMeter`, and `Interpreter::run`
- [Traces reference](/docs/reference/traces) for `Trace`, `TraceMeta`, and `Eval`
- [Program and nodes reference](/docs/reference/program-and-nodes) for `Program` and `ir::Overlay`
- [Bake a candidate](/docs/guides/bake-a-candidate)
