...
| `fx::predict("name", input)` | Functional lane: a named predictor per call slot, see [fx](/docs/reference/fx) |
...
| `.demo(example)` | Appends one `Example<S>` few-shot demo |
...
| `.lm(lm)` | Per-instance [LM](/docs/reference/lm), bypassing the global `configure()` LM |
...
A `ToolSet::code_mode(...)` set drops into `LM::call_with_toolset` directly. See [Code Mode](/docs/reference/code-mode).

## See also

- [Signatures and types](/docs/reference/signatures-and-types)
- [Modules](/docs/reference/modules)
- [LM](/docs/reference/lm)
- [Traces](/docs/reference/traces)
- [Code Mode](/docs/reference/code-mode)
