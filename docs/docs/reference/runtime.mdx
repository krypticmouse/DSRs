---
title: "Runtime"
description: "Reference for RuntimeEnv, Interpreter load and run, Budget, ambient overlays, and every LoadError and RunError variant"
icon: "gears"
---

The runtime is what turns a `Program` into something that executes: the host supplies bindings through a `RuntimeEnv`, `Interpreter::load` checks everything up front, and `Interpreter::run` evaluates the program on an input map. This page lists the full surface.
