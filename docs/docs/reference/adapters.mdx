...
An adapter sits between your [signature](/docs/reference/signatures-and-types) and the LM. It formats typed inputs into prompts and parses LM responses back into typed outputs.
...
After parsing, the adapter runs [constraints](/docs/reference/signatures-and-types):
...
This is what a prompt looks like for a complex nested type (from [`examples/16-insurance-claim-prompt.rs`](https://github.com/darinkishore/DSRs/blob/main/crates/dspy-rs/examples/16-insurance-claim-prompt.rs)):
...
- [Language models](/docs/reference/lm)
