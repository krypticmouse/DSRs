...
Every call returns [`Predicted<Output>`](/docs/reference/predict): the output struct (accessible directly via `Deref`) plus `CallMetadata` (token counts, raw response, tool traces) via `.metadata()`. Errors are always `PredictError`.
...
To author a module: define a struct holding `Predict`/`ChainOfThought` fields, derive `facet::Facet` so the optimizer's walker can discover the `Predict` leaves, and implement `forward`. See [Pipelines are just structs](/docs/learn/pipelines-are-just-structs).
...
| Aspect | Detail |
|--------|--------|
| Construction | `ChainOfThought::<S>::new()` or `ChainOfThought::<S>::builder()` (a `PredictBuilder`, see [Predict](/docs/reference/predict)). |
...
Tools implement rig's `ToolDyn`. See [Give the model hands](/docs/learn/give-the-model-hands) and [Add tools](/docs/guides/add-tools).
...
- [Predict and PredictBuilder](/docs/reference/predict) for the leaf module and its builder methods
- [Signatures and types](/docs/reference/signatures-and-types) for `Signature`, `Schema`, and generated input/output structs
- [Macros](/docs/reference/macros) for the `Signature` and `Augmentation` derives
- [Optimizers](/docs/reference/optimizers) for how `Predict` leaves are discovered through modules
- [Who calls the model](/docs/learn/who-calls-the-model) for the call path from module to LM
