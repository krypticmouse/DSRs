...
fn derive_struct(input: &DeriveInput, data: &DataStruct) -> syn::Result<proc_macro2::TokenStream> {
    ...
    for field in &fields.named {
        ...
        let map_entry_info = map_entry_info(&field_name, &rendered_field, None, None, &field_attrs)?;
...
fn derive_unit_enum_as_union(
    ...
    for variant in &data.variants {
        ...
        let variant_attrs = parse_variant_attrs(&variant.attrs)?;
        let rendered_variant = variant_alias(&variant_attrs, rename_all, &variant_name);
        let literal = rendered_variant.clone().unwrap_or_else(|| variant_name.clone());

        literals.push(quote! { ::baml_bridge::baml_types::TypeIR::literal_string(#literal.to_string()) });
        let match_values = match_strings_for_variant(&variant_name, rendered_variant.as_deref());
        match_arms.push(quote! { #(#match_values)|* => Ok(Self::#variant_ident) });
    }
...
fn derive_data_enum(
    ...
    for variant in &data.variants {
        ...
        let parse_variant = variant_parse_tokens(
            variant,
            &variant_name,
            Some(&rendered_variant),
            rename_all,
...
fn match_type_ir(ty: &Type) -> syn::Result<proc_macro2::TokenStream> {
    ...
    if let Some((_key, value)) = is_string_map_type(ty) {
        ...
        return Ok(quote! { ::baml_bridge::baml_types::TypeIR::map(::baml_bridge::baml_types::TypeIR::string(), #value_ir) });
    }
...
fn field_alias(attrs: &FieldAttrs, rename_all: Option<RenameRule>, field_name: &str) -> Option<String> {
...
fn map_entry_class_tokens(
    ...
    let name_expr = name_tokens_expr(entry.internal_name_expr.clone(), entry.rendered_name.as_deref());
...
fn variant_alias(attrs: &VariantAttrs, rename_all: Option<RenameRule>, name: &str) -> Option<String> {
    if let Some(alias) = &attrs.alias {
        return Some(alias.clone());
    }
...
fn parse_container_attrs(attrs: &[Attribute]) -> syn::Result<ContainerAttrs> {
    ...
    for attr in attrs {
        if attr.path().is_ident("baml") {
            parse_baml_meta(attr, |meta| match meta {
        ...
        if attr.path().is_ident("serde") {
            parse_serde_meta(attr, |meta| match meta {
...
fn parse_meta_list(attr: &Attribute) -> syn::Result<Vec<Meta>> {
    match attr.parse_args_with(syn::punctuated::Punctuated::<Meta, syn::Token![,]>::parse_terminated) {
        Ok(list) => Ok(list.into_iter().collect()),
        Err(err) => Err(err),
    }
}

fn parse_constraint(meta: syn::MetaList, level: ConstraintLevelSpec) -> syn::Result<ConstraintSpec> {
    let nested = meta.parse_args_with(syn::punctuated::Punctuated::<Meta, syn::Token![,]>::parse_terminated)?;
    let mut label = None;
    let mut expr = None;
    ...
    let label = label.ok_or_else(|| {
        syn::Error::new(meta.span(), "constraint missing label; hint: use label = \"...\"")
    })?;
    let expr = expr.ok_or_else(|| {
        syn::Error::new(meta.span(), "constraint missing expr; hint: use expr = \"...\"")
    })?;
...
fn parse_int_repr(expr: &Expr, span: proc_macro2::Span) -> syn::Result<IntRepr> {
    ...
    match value.as_str() {
        ...
        "i64" => Ok(IntRepr::I64),
        _ => Err(syn::Error::new(span, "int_repr must be \"string\" or \"i64\"")),
    }
}
...
fn is_string_type(ty: &Type) -> bool {
    type_ident(ty).map(|ident| ident == "String").unwrap_or(false)
}
    ...
    if let Type::Path(path) = ty {
        if let Some(segment) = path.path.segments.last() {
            if segment.ident == "Value" {
                return path.path.segments.iter().any(|seg| seg.ident == "serde_json");
            }
        }
    }
    false
